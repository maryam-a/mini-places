...
from DataLoader_modified import *
...
training_iters = 100000 #370000
step_display = 50 
step_save = 10000
path_save = 'vgg16results_4/'
start_from = ''
CONV_STRIDE = [1,1,1,1]
...
loader_test = DataLoaderDisk_test(**opt_data_test)
...
with tf.Session() as sess:
    ...
    while step < training_iters:
        ...
        if step % step_display == 0:
            ...
            training_data_holder.append((l, acc1, acc5))
            ...
            validation_data_holder.append((l, acc1, acc5))
    ...
    num_batch = loader_test.size()

    loader_test.reset()
    for i in range(num_batch):
        images_batch = loader_test.next_batch(1)
        result = sess.run([top5_pred], feed_dict = {x: images_batch, keep_dropout: 1.})[0][1][0]
        print(result) 

    print('####')
    print(training_data_holder)
    print(validation_data_holder)
